...
你不得直接修改 production 核心 pipeline。正式模型、temperature 與版本尚未凍結，不得啟動 12×4 正式批次。
...
第一輪不要修改檔案。等待成員回覆「確認開工」後只製作 runner、離線測試與一個 profile×4 條件 fake dry-run。正式模型尚未凍結，禁止執行 12×4 正式批次。
```
