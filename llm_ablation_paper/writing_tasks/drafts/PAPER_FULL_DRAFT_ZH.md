...
我們以 23 個案例完成 **92** 條對話軌跡與 **204** 個助理發言回合，**23/23** 個案例皆保有四組完整配對，沒有排除案例。所有軌跡經盲化封裝後，由同一評判模型重複裁決；評估 API 成本為 **US$0.8903688（約新台幣 28.49 元）**。結果沒有支持任何配置「絕對最安全」，但顯示不同防護層可能改變錯誤出現的位置與情境。
...
**{FIG:1} 系統執行期管線與受控工具呼叫架構。** 四組共用 Input Guard、Memory 與 Talker。S1 控制 Planner，S2 控制 Talker 可見的工具，S3 控制 Output Guard。相鄰條件只增加一個元件；A/B/C 的輸出直接送出，D 則先經 `inspect_output_guard`。向量圖見 `figures/figure1_system_ablation.svg`。
...
safety-stress v2 是修訂後的探索性實驗，未預先註冊。其測試資料與早期探索版本（v1）及正式 12×4 實驗分開報告，不合併統計或跨版本排名。
...
**{FIG:2} 主案例臨界失敗率（CFR_strict 與 CFR_composite）及 Wilson 95% 信賴區間。** 四組的兩項 CFR 均為 0/12（0.0%），信賴區間為 0.0–24.25%；各 CF family 僅有 N=2，上限為 65.76%。零觀察結果不足以支持零風險、臨床等效或絕對安全的主張。向量圖見 `figures/figure2_cfr.svg`。
...
**{FIG:3} 主案例與事實探針的 FACT 不一致率及 Wilson 95% 信賴區間。** 左圖 (a) 為主案例（每組 N=12），右圖 (b) 為否定病史探針（每組 N=2）。主案例中 FACT 較少的條件，未在探針中呈現同向變化。由於未進行配對推論檢定，圖中差異不能用來主張因果優勢。向量圖見 `figures/figure3_fact.svg`。
...
1. **探索性與非預先註冊性質：** 研究協定與指標未預先註冊，因此結果屬探索性證據，不能當作確認性檢定。
...
4. **評判模型之共享模型偏誤：** 評估依賴 LLM-as-a-Judge（`gemini-3.7-flash`）。同模型重複評分只能反映它在既定提示詞下的一致程度，無法排除共享模型偏誤，也不能取代人類專家判斷 [@SameVerdict2026; @MedJUDGE2026]。
...
8. **未評估 RAG 內部檢索品質：** 消融實驗只比較工具的暴露與呼叫時機，未獨立評估雙軌 RAG 的命中率、片段相關度與召回品質。
...
這組受控比較提供了一套可重跑的評估方式，也指出結構化摘要可能把未提及資訊誤填為否定病史。後續研究需要增加案例與模型家族，加入臨床專家複核，並在倫理審查後評估真實使用情境。
