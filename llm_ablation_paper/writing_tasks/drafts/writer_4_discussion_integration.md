...
5. **部分解盲限制：** 對話歷史中的 `tools_exposed` 讓評判模型可能區分 {Base, Planner} 與 {Planner + Gate, Full-stack}，因此評估並非完全盲測。
6. **固定腳本：** 病患台詞預先固定，未使用動態 Patient Agent，不能完整模擬真實病患的語言變化。
7. **單一模型版本：** Talker 與 Planner 均使用 `gemini-3.5-flash-lite`，溫度分別為 0.3 與 0.1，結果未必適用於其他模型。
8. **未評估 RAG 內部檢索品質：** 消融實驗只比較工具的暴露與呼叫時機，未獨立評估雙軌 RAG 的命中率、片段相關度與召回品質。
9. **Scanner 並非真值：** Scanner 是決定性啟發式規則，其與 Judge 的分歧不能直接判定為 Scanner 誤報或 Judge 漏判。
10. **外部效度有限：** 案例、用藥指引與用語以臺灣醫療及繁體中文情境設計，不宜直接外推至其他醫療體系或文化。
11. **部分案例事實未經臨床查證：** 部分藥物別名與藥理欄位標記為 `UNVERIFIED`，只能作為模擬案例設定，不能引用為臨床事實。
