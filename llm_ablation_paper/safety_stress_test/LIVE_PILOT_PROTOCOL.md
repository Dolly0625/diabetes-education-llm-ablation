...
- `--live-pilot` 啟動前必須同時滿足：
  ...
  3. 工作樹乾淨；
  4. 相對 stress tag 之變更全部落在 `llm_ablation_paper/safety_stress_test/`；
  5. 五項 frozen fingerprint 一致、case/schema 通過、A–D 唯一差異、tool gate 可達、provider 就緒。
...
- 輸出僅允許寫入使用者指定且 **gitignored** 的 live root（預設 `safety_stress_test/artifacts/live_pilot/`）。
